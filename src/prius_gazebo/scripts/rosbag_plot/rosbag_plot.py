...
def rosbag_callback(pose, i):
    ...
    if abs(pose.twist.twist.linear.y) > 0.01 or abs(pose.twist.twist.linear.x) > 0.01:
...
def animate_plot(i):
    ...
    y0 = map(abs, list(vel_profile_0[:,1]))
    y1 = map(abs, list(vel_profile_1[:,1]))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'save':
        ani.save('/home/liuyc/moby_folder/Research/figures/intersection_simulator/gif/lukc_liuyc_4_posevel.gif', writer='imagemagick', fps=4)
